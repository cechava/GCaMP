function dataAnalysis_tWindow_M4_minusRollingMean_perRun(projectID,sessID,run,tWindow1Start_ASO,tWindow1End_ASO,tWindow2Start_ASO,tWindow2End_ASO)
%method (M4): take maximum fractional change in t2 window
...
outDir=[analysisFolder,'Analysis_tWindow_M4_minusRollingMean_dsFactor',num2str(dsFactor),'_fwhm',num2str(fwhm),'/MidProcOutput/'];
if isdir(outDir)==0
    mkdir(outDir)
end
...
for i =1:length(stimStart)
    ...
    tWindow2Val=max(detrendedFrameArray(:,startInd:endInd),[],2); 
