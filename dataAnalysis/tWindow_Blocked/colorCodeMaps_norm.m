...
%analysisID=['Analysis_tWindow_M1_minusRollingMean2_dsFactor',num2str(dsFactor),'_fwhm',num2str(fwhm)];
...
outDir=[analysisFolder,analysisID,'/Figures/AllRuns/fromFracChange/'];
...
for c=1:nContrasts
    ...
    %normalize map
    map=(map-min(abs(map(:))))/(max(abs(map(:)))-min(abs(map(:))));
    allMaps(:,:,c)=map;
end
...
colorCode=[255 127 0; 255 255 0; 127 255 0;...
    0 127 127;0 255 255;0 127 255;...
    127 0 255; 125 0 125; 255 0 127];
...
imwrite(uint8(codedImg),[outDir,'colorCodedPositionMap_NormalizedMaps_thresh',num2str(thresh),'.tif'],'TIFF')
